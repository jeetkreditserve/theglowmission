...
class ContactIngestionTests(TestCase):
    ...
    def test_matching_updates_latest_non_empty_values_and_ignores_blanks(self):
        first = self.submit({"full_name": "Asha Rao", "email": "asha@example.com", "phone": "9876543210", "marketing_consent": True})
        second = self.submit({"full_name": "Asha R.", "email": "asha@example.com", "phone": "", "marketing_consent": False})
    ...
    def test_response_without_identity_is_saved_without_contact(self):
        response = self.submit({"marketing_consent": True})

        self.assertIsNone(response.contact)
        self.assertEqual(response.contact_sync_status, "skipped")
